...
char **argv;
{
    ...
    nreq = 0;
    if (rank < size - 1) {
        MPI_Isend(xlocal[maxn / size], maxn, MPI_DOUBLE, rank + 1, 0, MPI_COMM_WORLD, &r[nreq++]); /* Statement S7 */
    }
    /* Receive from previous unless I'm the first process (rank == 0)
       and store row in initial row (ghost area) */
    if (rank > 0) {
        MPI_Irecv(xlocal[0], maxn, MPI_DOUBLE, rank - 1, 0, MPI_COMM_WORLD, &r[nreq++]); /* Statement S8 */
    }

    /* Send 1st local row to previous process unless I'm the 1st process */
    if (rank > 0) {
        MPI_Isend(xlocal[1], maxn, MPI_DOUBLE, rank - 1, 1, MPI_COMM_WORLD, &r[nreq++]); /* Statement S9 */
    }
    /* Receive from next process and store row in last row in xlocal
       (ghost area) unless I'm the last process */
    if (rank < size - 1) {
        MPI_Irecv(xlocal[maxn / size + 1], maxn, MPI_DOUBLE, rank + 1, 1, MPI_COMM_WORLD, &r[nreq++]); /* Statement S10 */
    }
    ...
    MPI_Reduce(&errcnt, &toterr, 1, MPI_INT, MPI_SUM, 0, MPI_COMM_WORLD); /* Statement S11 */
