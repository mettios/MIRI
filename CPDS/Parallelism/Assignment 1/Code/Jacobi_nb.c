...
int main(int argc, char **argv) {
    ...
    i_first = 1;
    i_last = maxn / size;
    /* Note that first and last processes have one less row of interior points */
    if (rank == 0) i_first++;
    ...
    do {
        ...
        MPI_Isend(xlocal[maxn / size], maxn, MPI_DOUBLE, next_nbr, 0, MPI_COMM_WORLD, &r[0]);
        /* Receive from previous unless I'm the first process (rank==0) */
        MPI_Irecv(xlocal[0], maxn, MPI_DOUBLE, prev_nbr, 0, MPI_COMM_WORLD, &r[2]);

        /* Send 1st local row to previous process unless I'm the 1st process */
        MPI_Isend(xlocal[1], maxn, MPI_DOUBLE, prev_nbr, 1, MPI_COMM_WORLD, &r[3]);
        /* Receive from next process */
        MPI_Irecv(xlocal[maxn / size + 1], maxn, MPI_DOUBLE, next_nbr, 1, MPI_COMM_WORLD, &r[1]);
        ...
        MPI_Iallreduce(&diffnorm, &gdiffnorm, 1, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD, &r[0]); /* Statement S19 */
        MPI_Wait(&r[0], &status);
    ...
    MPI_Igather(xlocal[1], maxn * (maxn / size), MPI_DOUBLE, x, maxn * (maxn / size), MPI_DOUBLE, 0, MPI_COMM_WORLD, &r[0]); /* Statement S20 */
    MPI_Wait(&r[0], &status);
    if (rank == 0) {
