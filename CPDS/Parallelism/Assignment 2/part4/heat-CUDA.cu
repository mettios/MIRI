...
int main( int argc, char *argv[] ) {
    ...
    float *residuals, *dev_residuals;
    // Allocation on GPU for matrices u and uhelp
    cudaMalloc(&dev_u, np * np * sizeof(float));
    cudaMalloc(&dev_uhelp, np * np * sizeof(float));
    cudaMalloc(&dev_residuals, Grid_Dim * Grid_Dim * sizeof(float));
    residuals = (float*)malloc(Grid_Dim * Grid_Dim * sizeof(float));
    ...
    iter = 0;
    while(1) {
        gpu_Heat<<<Grid,Block,Block_Dim*Block_Dim*sizeof(float)>>>(dev_u, dev_uhelp, np, dev_residuals);
        cudaDeviceSynchronize();  // Wait for compute device to finish.

        // Residuals are computed on device, need to copy them for later
	cudaMemcpy(residuals, dev_residuals, Grid_Dim * Grid_Dim * sizeof(float), cudaMemcpyDeviceToHost);
...
//	residual = cpu_residual (param.u, param.uhelp, np, np);
	...
	for (int i = 0; i < Grid_Dim*Grid_Dim*sizeof(float); i++)
